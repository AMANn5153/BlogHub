...
const getBlog = asyncHandler(async (req, res, next) => {
    ...
    const likeOnBlog = await Likes.find({blogId : blogExists._id, commentId : null});
    
    const savesOnBlog = await Saves.find({blogId : blogExists._id});
...
const newBlog = async (req, res, next) =>{
    try{    
        ...
        const lastEndOfId = blog._id.toString().slice(-5);
        const slug = slugify(blog.heading +" "+ lastEndOfId);
        const updateWithSlug = await Blogs.findOneAndUpdate({_id : blog._id}, {slug : slug}, {new : true});
